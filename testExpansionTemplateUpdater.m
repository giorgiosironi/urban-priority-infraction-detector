...
foreground = ones(6) * -1;
foreground(3:4, 1:4) = 1;
foregroundObjects = {TrackedObjectPosition({Patch(NaN, Area.fromXYtoXY(3, 1, 4, 2)); Patch(NaN, Area.fromXYtoXY(3, 3, 4, 4))})};
...
newHistograms = histogramFactory.buildFromImage(nextFrame);
foreground = ones(6) * -1;
foregroundObjects = {TrackedObjectPosition({Patch(NaN, Area.fromXYtoXY(3, 3, 4, 4))})};
